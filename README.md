...
Phase 19 stores verified online backups under `~/.multiagents/backups/` with
directory mode `0700` and file mode `0600`. Backups never include provider or
Slack credentials. Restore is offline-only via
`npm run state:restore -- <backup-id>` and retains the previous state database.
Code-only rollback after a schema migration is unsupported; restore the
matching pre-upgrade backup together with the matching application revision.
...
- Every browser mutation uses one common human gate: the exact localhost origin and Host, `Sec-Fetch-Site: same-origin`, an action-specific header, and a short-lived single-use nonce tied to an HttpOnly SameSite browser-session cookie. Nonces are kept only as hashes in process memory, are never stored in SQLite or audit output, and cannot be issued or used while an agent is active.
- Rerun requests accept only a persisted `taskId` and fixed `stepId`. The server reconstructs the prompt, flow ID, canonical agent/role, prior outputs, profile/template policy, cwd, and stale dependencies from task state; client prompt, agent, role, flow, and output overrides are rejected.
...
- The approval hash covers the base commit plus NUL-safe canonical Git status, rename/copy old and new paths, final file content, file mode, symlink target, deletion state, and tracked/untracked additions. Content that exceeds the review limits or cannot be fully shown is not approvable.
...
- Server Git mutations use fixed binaries and arguments, `shell: false`, an environment without caller-controlled `GIT_CONFIG_*`, `GIT_DIR`, `GIT_WORK_TREE`, or `GIT_INDEX_FILE`, no system/global Git config, and `core.hooksPath=/dev/null`. Unsafe local configuration is rejected. After staging, MultiAgents fixes the index tree ID; after commit it verifies the parent, committed tree, approved delta, and clean worktree before any push or PR operation.
- Cursor review runs in fixed ask/read-only sandbox mode with worktree setup scripts disabled. Claude review uses the installed CLI's restricted and safe modes, strict MCP isolation, disabled custom commands/session persistence/Chrome integration, no permission prompting, and only Read/Glob/Grep tools. Cursor authentication and customization do not currently have separable config roots, so private runtime HOME isolation remains a Phase 18 requirement.
