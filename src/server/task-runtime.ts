...
export async function prepareTaskRuntime(task: RepoTask) {
  ...
  return {
    policies,
    execute: (adapter: AgentAdapter, prompt: string, signal?: AbortSignal, stepId?: string, onProviderWorkStart?: () => void, onChildClose?: () => void): Promise<AgentResult> => {
    ...
    executePolicy: (adapter: AgentAdapter, policy: RuntimePolicy, prompt: string, signal?: AbortSignal, stepId?: string, onProviderWorkStart?: () => void, onChildClose?: () => void): Promise<AgentResult> => {
      return executePolicy(adapter, policy, prompt, signal, stepId, onProviderWorkStart, onChildClose);
    },
  };

  function executePolicy(adapter: AgentAdapter, policy: RuntimePolicy, prompt: string, signal?: AbortSignal, stepId?: string, onProviderWorkStart?: () => void, onChildClose?: () => void) {
...
export function taskRuntimeExecutor(runtime: Awaited<ReturnType<typeof prepareTaskRuntime>>, adapters: Record<AgentId, AgentAdapter>) {
  return (agent: AgentId, prompt: string, signal: AbortSignal, stepId: string, onProviderWorkStart?: () => void, onChildClose?: () => void) => runtime.execute(adapters[agent], prompt, signal, stepId, onProviderWorkStart, onChildClose);
}
