...
function TaskCard({ task, busy, onResume, onHistory, onRefreshPr, onCleanup }: { task: DashboardTask; busy: boolean; onResume: Props["onResume"]; onHistory: Props["onHistory"]; onRefreshPr: (task: DashboardTask) => void; onCleanup: (task: DashboardTask) => void }) {
  return <article className={`dashboardCard bucket-${task.bucket}`}>
    ...
    <dl className="taskFacts"><div><dt>Branch</dt><dd><code>{task.branch}</code></dd></div><div><dt>PR</dt><dd>{task.prNumber ? `#${task.prNumber}${task.prState ? ` · ${task.prState}` : ""}` : "None"}</dd></div><div><dt>Recovery</dt><dd>{task.recoveryStatus}</dd></div><div><dt>Worktree</dt><dd>{task.worktreeStatus}</dd></div><div><dt>Updated</dt><dd>{relativeTime(task.updatedAt)}</dd></div></dl>
