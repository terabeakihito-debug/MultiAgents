...
export function RemediationQueue({ repos, busy, refreshToken, onOpenTask, onHistory, onError }: Props) {
  ...
  return <section className="remediationQueue" aria-labelledby="remediation-queue-title">
    ...
    <div className="remediationCounts" aria-label="Findings summary">
    ...
    <div className="findingsFilters">
      ...
      <details className="advancedFindingFilters"><summary>Advanced filters</summary><div>
    ...
    {loading ? <p className="muted">Loading findings…</p> : data.findings.length ? <div className="queueTableWrap"><table className="queueTable"><thead><tr><th>Severity</th><th>Finding</th><th>Repository</th><th>Stage</th><th>Next action</th></tr></thead><tbody>{data.findings.map((finding) => <QueueRow key={`${finding.findingId}-${finding.humanPriority}`} finding={finding} busy={busy} onOpenTask={onOpenTask} onHistory={onHistory} onError={onError} onChanged={refresh} />)}</tbody></table></div> : <p className="emptyDashboard">No findings match these filters.</p>}
  </section>;
}
...
function QueueRow({ finding, busy, onOpenTask, onHistory, onError, onChanged }: { finding: RemediationQueueItem; busy: boolean; onOpenTask: Props["onOpenTask"]; onHistory: Props["onHistory"]; onError: Props["onError"]; onChanged: () => void }) {
  ...
  return <>
    ...
    {expanded ? <tr className="queueDetail"><td colSpan={5}><dl><div><dt>Finding ID</dt><dd><code>{finding.findingId}</code></dd></div><div><dt>Priority</dt><dd><div className="priorityEditor"><select aria-label={`Human priority for ${finding.title}`} value={selectedPriority} disabled={busy || processing || Boolean(finding.resolvedAt)} onChange={(event) => setSelectedPriority(event.target.value as HumanPriority)}>{humanPriorities.map((value) => <option key={value} value={value}>{value}</option>)}</select><button type="button" className="compactButton" disabled={busy || processing || selectedPriority === finding.humanPriority || Boolean(finding.resolvedAt)} onClick={() => void savePriority()}>Save</button></div></dd></div><div><dt>Implementation</dt><dd>{finding.implementationTaskId ? <><code>{finding.implementationTaskId.slice(0, 8)}</code> · {finding.implementationTaskStatus}</> : "Not created"}</dd></div><div><dt>Pull request</dt><dd>{finding.prNumber ? finding.prUrl ? <a href={finding.prUrl} target="_blank" rel="noreferrer">#{finding.prNumber}</a> : `#${finding.prNumber}` : "None"}</dd></div><div><dt>Age</dt><dd>{age(finding.ageMs)}</dd></div><div><dt>Source</dt><dd><button type="button" className="textButton" onClick={() => onHistory(finding.sourceTaskId, `${finding.repoName} — ${finding.title}`)}>Task history</button> · {finding.sourceTemplateName || "Unknown template"} v{finding.sourceTemplateVersion ?? "?"}</dd></div><div><dt>Finding status</dt><dd>{finding.findingStatus}</dd></div><div><dt>Updated</dt><dd>{new Date(finding.updatedAt).toLocaleString()}</dd></div></dl>{finding.attentionReason ? <p><strong>Details:</strong> {finding.attentionReason}</p> : null}{finding.affectedPaths?.length ? <p><strong>Affected paths:</strong> {finding.affectedPaths.join(", ")}</p> : null}</td></tr> : null}
  </>;
}
