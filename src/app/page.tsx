...
export default function Home() {
  ...
  return <main>
    ...
    <form onSubmit={submit}>
      ...
      <div className="actions"><span>Step 4</span><button type="button" disabled={!repoId || !selectedTemplate || !prompt.trim() || sending || selectedProfile?.enabled === false} onClick={createIsolatedTask}>{sending ? "Starting…" : "Start task"}</button></div>
...
function StartTaskPanel({ repos, repoId, templateId, prompt, sending, profileEnabled, selectedRepo, onRepo, onTemplate, onPrompt, onStart, onProjectAdded, onError }: { repos: Repo[]; repoId: string; templateId: string; prompt: string; sending: boolean; profileEnabled: boolean; selectedRepo?: Repo; onRepo: (id: string) => void; onTemplate: (id: string) => void; onPrompt: (value: string) => void; onStart: () => void; onProjectAdded: (project: AddedProject, needsInitialCommit: boolean) => void; onError: (error: string) => void }) {
  const selectedTemplate = selectedRepo?.templates.find((template) => template.templateId === templateId);
  const initializationBlocked = selectedRepo?.initializationRequired || selectedRepo?.initializationRepairRequired;
  return <section className="startTaskWide" aria-labelledby="start-task-title"><div className="startTaskHeading"><div><span className="eyebrow">Start a new task</span><h2 id="start-task-title">What would you like to work on?</h2></div><p>Choose a repository and task type, then describe the work.</p></div><div className="startFields"><label>Repository<div className="repositorySelect"><select value={repoId} disabled={sending} onChange={(event) => onRepo(event.target.value)}>{repos.map((repo) => <option key={repo.id} value={repo.id}>{repo.name}{repo.dirty ? " (needs attention)" : ""}</option>)}</select><ProjectOnboarding disabled={sending} onAdded={onProjectAdded} onError={onError} initializationRequiredProject={initializationBlocked ? selectedRepo : undefined} initializationRepairRequired={selectedRepo?.initializationRepairRequired === true} /></div></label><label>Task type<select value={templateId} disabled={sending} onChange={(event) => onTemplate(event.target.value)}>{selectedRepo?.templates.filter((template) => template.enabled).map((template) => <option key={template.templateId} value={template.templateId}>{template.name}</option>)}</select><small>{selectedTemplate?.description}{selectedTemplate?.readOnly ? " Read-only: no worktree, commit, or PR." : ""}</small></label></div><label className="taskDescription">Task description<textarea value={prompt} onChange={(event) => onPrompt(event.target.value)} maxLength={20_000} rows={4} placeholder="Describe the task, desired outcome, and any constraints." /></label><div className="startTaskFooter"><details><summary>Advanced settings</summary><p>Profiles and technical task settings are available in Settings. Safe server-side policies remain enforced.</p></details><button type="button" disabled={!repoId || !selectedTemplate || !prompt.trim() || sending || !profileEnabled || initializationBlocked} onClick={onStart}>{initializationBlocked ? "Complete project setup first" : sending ? "Starting…" : "Start task"}</button></div></section>;
}
...
function TaskDetail({ task, tab, onTab, onDelete, onRun, busy }: { task: RepoTask; tab: "overview" | "changes" | "history" | "technical"; onTab: (tab: "overview" | "changes" | "history" | "technical") => void; onDelete: () => void; onRun: () => void; busy: boolean }) {
  ...
  return <section className="taskDetail card" aria-labelledby="task-detail-title"><div className="cardHeader"><div><span className="eyebrow">Task</span><h2 id="task-detail-title">{task.prompt}</h2></div><Status value={task.status} /></div>
    ...
    {tab === "technical" ? <details open><summary>Technical details</summary><p>Repository: {task.repoName} · Branch: <code>{task.branch}</code> · Profile: {task.profile.name} v{task.profile.version} · Task type snapshot: {task.template.name} v{task.template.version}</p><button type="button" className="danger" onClick={onDelete} disabled={busy || Boolean(task.commitSha) || !task.worktreeAvailable}>Delete task worktree</button></details> : null}
  </section>;
}
