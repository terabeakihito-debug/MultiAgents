...
export default function Home() {
  ...
  const [dashboardView, setDashboardView] = useState<"tasks" | "findings" | "operations">("tasks");
  ...
  return <main>
    <header className="appHeader"><div><h1>MultiAgents</h1><p>Parallel answers or a fixed, reviewed response from local AI CLIs.</p></div><NotificationCenter repos={repos} refreshToken={dashboardRefresh} onOpenTask={(id) => { setDashboardView("tasks"); void resumePersistedTask(id); }} onError={setTaskError} /></header>
    <TaskDashboard repos={repos} busy={sending || approvalProcessing || reviewProcessing} refreshToken={dashboardRefresh} view={dashboardView} onViewChange={setDashboardView} onResume={(id) => { setDashboardView("tasks"); void resumePersistedTask(id); }} onHistory={(id, label) => { setDashboardView("tasks"); void openDashboardHistory(id, label); }} onError={setTaskError} />
    {dashboardView === "tasks" ? <>
