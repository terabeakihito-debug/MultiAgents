...
export default function Home() {
  ...
  return <main>
    ...
    {task && dependencyRecoveryPresentation(task.dependencyRecovery) ? <section className="dependencyRecovery" aria-labelledby="dependency-recovery-title"><span className="eyebrow">Validation recovery</span><h3 id="dependency-recovery-title">Dependencies required / 依存関係の準備が必要</h3><p><strong>次にすること:</strong> {task.status === "draft" && task.flowStatus === "error" ? "依存関係をインストール済みなら「レビューを再実行」を押してください。レビュー完了後、Changesで差分を確認して承認します。" : "依存関係の準備後、「承認確認をやり直す」を押し、差分を確認して承認します。"}</p><button type="button" className="secondary" disabled={approvalProcessing || sending || setupCommandLoading} onClick={() => void showSetupCommand(task)}>{setupCommandLoading ? "Loading setup command…" : "セットアップコマンドを表示"}</button>{setupCommand ? <div><p><strong>MultiAgents does not run this command.</strong> WSLで実行するコマンドを確認してください。</p><pre><code>{setupCommand}</code></pre></div> : null}{task.status === "draft" && task.flowStatus === "error" ? <button type="button" disabled={approvalProcessing || sending} onClick={() => void runTaskFlow()}>{sending ? "Starting…" : "レビューを再実行 (Retry review)"}</button> : <button type="button" className="secondary" disabled={approvalProcessing || sending} onClick={() => void refreshDiff(task)}>承認確認をやり直す (Recheck approval snapshot)</button>}</section> : null}
