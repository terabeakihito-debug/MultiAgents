...
import { historyEventStatusLabel, statusBadgeLabel, taskBehaviorExplanation, taskStatusLabel } from "./task-labels";
...
export default function Home() {
  ...
  async function createIsolatedTask(resolvedTemplateId: string) {
    ...
    await submitTaskStart({
      ...
      request: async (form) => {
        ...
        const response = await humanMutationFetch("/api/tasks", "task-create", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ repoId: form.repoId, templateId: form.templateId, prompt: form.prompt, autonomous: autonomous && !selectedTemplate?.readOnly }) });
  ...
  return <main>
    ...
    </form></section> : task ? <TaskDetail task={task} tab={detailTab} onTab={setDetailTab} onDelete={deleteWorktree} onRun={() => void runTaskFlow()} busy={sending || approvalProcessing || reviewProcessing} /> : null}
    ...
    {task && dependencyRecoveryPresentation(task.dependencyRecovery) ? <section className="dependencyRecovery" aria-labelledby="dependency-recovery-title"><span className="eyebrow">Validation recovery</span><h3 id="dependency-recovery-title">Dependencies required</h3><p>{task.status === "draft" && task.flowStatus === "error" ? "The review flow ended before approval. Install dependencies, then retry the review." : "Pre-PR validation stopped because this task worktree does not have a complete local dependency tree."}</p><button type="button" className="secondary" disabled={approvalProcessing || sending || setupCommandLoading} onClick={() => void showSetupCommand(task)}>{setupCommandLoading ? "Loading setup command…" : "Show setup command"}</button>{setupCommand ? <div><p><strong>MultiAgents does not run this command.</strong> Package installation may execute lifecycle scripts. Review the command before running it.</p><pre><code>{setupCommand}</code></pre></div> : null}{task.status === "draft" && task.flowStatus === "error" ? <button type="button" disabled={approvalProcessing || sending} onClick={() => void runTaskFlow()}>{sending ? "Starting…" : "Retry review"}</button> : <button type="button" className="secondary" disabled={approvalProcessing || sending} onClick={() => void refreshDiff(task)}>Recheck approval snapshot</button>}</section> : null}
...
function StartTaskPanel({ repos, repoId, templateId, prompt, autonomous, sending, profileEnabled, selectedRepo, resolution, taskStartError, onRepo, onTemplate, onPrompt, onAutonomous, onStart, onProjectAdded, onError, onOpenSettings }: { repos: Repo[]; repoId: string; templateId: string; prompt: string; autonomous: boolean; sending: boolean; profileEnabled: boolean; selectedRepo?: Repo; resolution: "override" | "default" | "only_enabled" | "selection_required" | "none_enabled"; taskStartError: string; onRepo: (id: string) => void; onTemplate: (id: string) => void; onPrompt: (value: string) => void; onAutonomous: (value: boolean) => void; onStart: () => void; onProjectAdded: (project: AddedProject, needsInitialCommit: boolean) => void; onError: (error: string) => void; onOpenSettings: () => void }) {
...
function TaskDetail({ task, tab, onTab, onDelete, onRun, busy }: { task: RepoTask; tab: "overview" | "changes" | "history" | "technical"; onTab: (tab: "overview" | "changes" | "history" | "technical") => void; onDelete: () => void; onRun: () => void; busy: boolean }) {
  ...
  return <section className="taskDetail card" aria-labelledby="task-detail-title"><div className="cardHeader"><div><span className="eyebrow">Task</span><h2 id="task-detail-title">{task.prompt}</h2></div><Status domain="task" value={task.status} /></div>
    {readOnly ? <p className="readOnlyNotice"><strong>{task.template.name} · Read-only</strong><br />No worktree, commit, or PR. Complete the review, then view findings.</p> : task.autonomous ? <p className="muted">Autonomous mode: validation failure triggers one bounded repair retry. Human approval remains required.</p> : <p className="muted">Next action: <strong>{task.status === "draft" ? "Start review" : task.status === "awaiting_approval" ? "Review changes" : "Continue safely"}</strong></p>}
    <div className="detailTabs" role="tablist" aria-label="Task detail"><button type="button" className={tab === "overview" ? "selected" : "secondary"} onClick={() => onTab("overview")}>Overview</button><button type="button" className={tab === "changes" ? "selected" : "secondary"} onClick={() => onTab("changes")}>Changes</button><button type="button" className={tab === "history" ? "selected" : "secondary"} onClick={() => onTab("history")}>History</button><button type="button" className={tab === "technical" ? "selected" : "secondary"} onClick={() => onTab("technical")}>Technical</button></div>
    {tab === "overview" ? <><div className="progressList">{progress.map((item) => <span key={item}>{item}</span>)}</div>{dependencyRecovery ? <section className="dependencyRecovery" aria-labelledby="dependency-recovery-overview-title"><span className="eyebrow">Validation recovery</span><h3 id="dependency-recovery-overview-title">Dependencies required</h3><p>{retryReview ? "The review flow ended before approval. Install dependencies, then retry the review." : "Approval validation stopped because this task worktree needs local dependencies."}</p><button type="button" className="secondary" disabled={busy} onClick={() => onTab("changes")}>Open recovery actions</button>{retryReview ? <button type="button" disabled={busy} onClick={onRun}>{busy ? "Starting…" : "Retry review"}</button> : null}</section> : null}{task.status === "draft" && !dependencyRecovery ? <button type="button" disabled={busy} onClick={onRun}>{busy ? "Starting…" : "Start task"}</button> : null}{!readOnly && task.status === "awaiting_approval" ? <button type="button" onClick={() => onTab("changes")}>Review changes</button> : null}{readOnly && task.flowStatus === "completed" ? <button type="button" onClick={() => document.getElementById("findings-title")?.scrollIntoView()}>Show findings</button> : null}</> : null}
    {tab === "technical" ? <details open><summary>Technical details</summary><p>Repository: {task.repoName} · Branch: <code>{task.branch}</code> · Profile: {task.profile.name} v{task.profile.version} · Task type snapshot: {task.template.name} v{task.template.version}</p><p>内部状態: <code>{task.status}</code> · 復旧状態: <code>{task.recoveryStatus}</code> · 作業領域: <code>{task.worktreeStatus}</code></p><button type="button" className="danger" onClick={onDelete} disabled={busy || Boolean(task.commitSha) || !task.worktreeAvailable}>Delete task worktree</button></details> : null}
  </section>;
}
...
function PrReviewPanel({ task, processing, onFetch, onApply }: { task: RepoTask; processing: boolean; onFetch: () => void; onApply: () => void }) {
  ...
  return <section className="card prReview"><div className="cardHeader"><h2>PR Review Intake</h2><button type="button" disabled={processing || ["reworking", "reviewing_rework", "validating", "committing_rework", "pushing_rework", "checking_ci"].includes(task.status)} onClick={onFetch}>{processing ? "Working…" : review ? "Refresh review" : "Fetch review"}</button></div>
    {review && <><h3>Pull Request #{review.number}</h3><p><a href={review.url} target="_blank" rel="noreferrer">{review.title}</a></p><div className="prGrid"><span><strong>{review.state}</strong>{review.draft ? " · DRAFT" : ""}</span><span>Base: <code>{review.base}</code></span><span>Head: <code>{review.head}</code></span><span>SHA: <code>{review.headSha.slice(0, 12)}</code></span><span>Mergeable: {review.mergeable} / {review.mergeStateStatus}</span><span>Unresolved threads: {review.unresolvedCount}</span></div>
      <h3>Checks</h3>{review.checks.length ? review.checks.map((check) => <div className="validationRow" key={`${check.name}-${check.state}`}><span>{check.name}{check.required ? " (required)" : ""}</span><Status domain="pr_check" value={check.bucket} /><span>{check.state}</span></div>) : <p>No checks reported.</p>}
...
function ErrorBlock({ error }: { error: string }) { return <div className="error" role="alert"><strong>Error</strong><pre>{error}</pre></div>; }
