...
import { defaultFlowStepModels, type FlowStepModelPlan } from "@/flows/agent-models";
...
export default function Home() {
  ...
  const [stepModels, setStepModels] = useState<FlowStepModelPlan>(() => defaultFlowStepModels(defaultFlowStepAgents()));
  ...
  useEffect(() => {
    ...
    setStepAgents(agents);
    setStepModels(defaultFlowStepModels(agents));
  }, [repoId, templateId]);
  ...
  async function createIsolatedTask(resolvedTemplateId: string) {
    ...
    await submitTaskStart({
      ...
      request: async (form) => {
        ...
        const response = await humanMutationFetch("/api/tasks", "task-create", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ repoId: form.repoId, templateId: form.templateId, prompt: form.prompt, autonomous: autonomous && !selectedTemplate?.readOnly, stepAgents, stepModels }) });
...
function StartTaskPanel({ repos, repoId, templateId, prompt, autonomous, sending, profileEnabled, selectedRepo, resolution, taskStartError, stepAgents, stepModels, onStepAgents, onStepModels, onRepo, onTemplate, onPrompt, onAutonomous, onStart, onProjectAdded, onError, onOpenSettings }: { repos: Repo[]; repoId: string; templateId: string; prompt: string; autonomous: boolean; sending: boolean; profileEnabled: boolean; selectedRepo?: Repo; resolution: "override" | "default" | "only_enabled" | "selection_required" | "none_enabled"; taskStartError: string; stepAgents: FlowStepAgentPlan; stepModels: FlowStepModelPlan; onStepAgents: (plan: FlowStepAgentPlan) => void; onStepModels: (plan: FlowStepModelPlan) => void; onRepo: (id: string) => void; onTemplate: (id: string) => void; onPrompt: (value: string) => void; onAutonomous: (value: boolean) => void; onStart: () => void; onProjectAdded: (project: AddedProject, needsInitialCommit: boolean) => void; onError: (error: string) => void; onOpenSettings: () => void }) {
