...
function StartTaskPanel({ repos, repoId, templateId, prompt, sending, profileEnabled, selectedRepo, onRepo, onTemplate, onPrompt, onStart }: { repos: Repo[]; repoId: string; templateId: string; prompt: string; sending: boolean; profileEnabled: boolean; selectedRepo?: Repo; onRepo: (id: string) => void; onTemplate: (id: string) => void; onPrompt: (value: string) => void; onStart: () => void }) {
  const selectedTemplate = selectedRepo?.templates.find((template) => template.templateId === templateId);
  return <section className="startTaskWide" aria-labelledby="start-task-title"><div className="startTaskHeading"><div><span className="eyebrow">Start a new task</span><h2 id="start-task-title">What would you like to work on?</h2></div><p>Choose a repository and task type, then describe the work.</p></div><div className="startFields"><label>Repository<select value={repoId} disabled={sending} onChange={(event) => onRepo(event.target.value)}>{repos.map((repo) => <option key={repo.id} value={repo.id}>{repo.name}{repo.dirty ? " (needs attention)" : ""}</option>)}</select></label><label>Task type<select value={templateId} disabled={sending} onChange={(event) => onTemplate(event.target.value)}>{selectedRepo?.templates.filter((template) => template.enabled).map((template) => <option key={template.templateId} value={template.templateId}>{template.name}</option>)}</select><small>{selectedTemplate?.description}{selectedTemplate?.readOnly ? " Read-only: no worktree, commit, or PR." : ""}</small></label></div><label className="taskDescription">Task description<textarea value={prompt} onChange={(event) => onPrompt(event.target.value)} maxLength={20_000} rows={4} placeholder="Describe the task, desired outcome, and any constraints." /></label><div className="startTaskFooter"><details><summary>Advanced settings</summary><p>Profiles and technical task settings are available in Settings. Safe server-side policies remain enforced.</p></details><button type="button" disabled={!repoId || !selectedTemplate || !prompt.trim() || sending || !profileEnabled} onClick={onStart}>{sending ? "Starting…" : "Start task"}</button></div></section>;
}
