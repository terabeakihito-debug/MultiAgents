...
export default function Home() {
  ...
  return <main>
    ...
    {task && dependencyRecoveryPresentation(task.dependencyRecovery) ? <section className="dependencyRecovery" aria-labelledby="dependency-recovery-title"><span className="eyebrow">復旧</span><h3 id="dependency-recovery-title">依存関係の準備が必要</h3><p><strong>最初に確認:</strong> 「依存関係を確認」を押すと、作業ツリーの node_modules と npm ls --offline の結果を確認できます。npm install は実行しません。</p><div className="inlineActions"><button type="button" className="secondary" disabled={approvalProcessing || sending || setupCommandLoading || dependencyCheckLoading} onClick={() => void showSetupCommand(task)}>{setupCommandLoading ? "読み込み中…" : "セットアップ手順を表示"}</button><button type="button" className="secondary" disabled={approvalProcessing || sending || setupCommandLoading || dependencyCheckLoading} onClick={() => void checkDependencyReadiness(task)}>{dependencyCheckLoading ? "確認中…" : "依存関係を確認"}</button></div>{dependencyCheck ? <DependencyCheckResultView result={dependencyCheck} nextAction={task.status === "draft" && task.flowStatus === "error" ? "確認済みなら「レビューを再実行」を押してください。" : "確認済みなら「差分を再確認」を押してください。"} /> : null}{setupCommand ? <div><p><strong>このコマンドはMultiAgentsから自動実行されません。</strong> WSLで内容を確認して実行してください。</p><pre><code>{setupCommand}</code></pre></div> : null}{task.status === "draft" && task.flowStatus === "error" ? <button type="button" disabled={approvalProcessing || sending || dependencyCheck?.status !== "ready"} onClick={() => void runTaskFlow()}>{sending ? "開始中…" : "レビューを再実行"}</button> : <button type="button" className="secondary" disabled={approvalProcessing || sending || dependencyCheck?.status !== "ready"} onClick={() => void refreshDiff(task)}>差分を再確認</button>}</section> : null}
...
function TaskDetail({ task, tab, onTab, onDelete, onRun, onCheckDependencies, dependencyCheck, dependencyCheckLoading, busy }: { task: RepoTask; tab: "overview" | "changes" | "history" | "technical"; onTab: (tab: "overview" | "changes" | "history" | "technical") => void; onDelete: () => void; onRun: () => void; onCheckDependencies: () => void; dependencyCheck: DependencyReadinessCheck | null; dependencyCheckLoading: boolean; busy: boolean }) {
  ...
  return <section className="taskDetail card" aria-labelledby="task-detail-title"><div className="cardHeader"><div><span className="eyebrow">Task</span><h2 id="task-detail-title">{task.prompt}</h2></div><Status domain="task" value={task.status} /></div>
    ...
    {tab === "overview" ? <><div className="progressList">{progress.map((item) => <span key={item}>{item}</span>)}</div>{dependencyRecovery ? <section className="dependencyRecovery" aria-labelledby="dependency-recovery-overview-title"><span className="eyebrow">復旧</span><h3 id="dependency-recovery-overview-title">依存関係の準備が必要</h3><p><strong>最初に確認:</strong> 「依存関係を確認」で、作業ツリーの依存関係が使える状態か判断できます。npm install は実行しません。</p><div className="inlineActions"><button type="button" className="secondary" disabled={busy} onClick={onCheckDependencies}>{dependencyCheckLoading ? "確認中…" : "依存関係を確認"}</button><button type="button" className="secondary" disabled={busy} onClick={() => onTab("changes")}>復旧操作を開く</button></div>{dependencyCheck ? <DependencyCheckResultView result={dependencyCheck} nextAction={retryReview ? "確認済みなら「レビューを再実行」を押してください。" : "確認済みなら「変更内容」で差分を再確認してください。"} /> : null}{retryReview ? <button type="button" disabled={busy || dependencyCheck?.status !== "ready"} onClick={onRun}>{busy ? "開始中…" : "レビューを再実行"}</button> : null}</section> : null}{task.status === "draft" && !dependencyRecovery ? <button type="button" disabled={busy} onClick={onRun}>{busy ? "開始中…" : "タスクを開始"}</button> : null}{!readOnly && task.status === "awaiting_approval" ? <button type="button" onClick={() => onTab("changes")}>変更内容を確認</button> : null}{readOnly && task.flowStatus === "completed" ? <button type="button" onClick={() => document.getElementById("findings-title")?.scrollIntoView()}>指摘を表示</button> : null}</> : null}
