  ...
  async function createIsolatedTask(resolvedTemplateId: string) {
    ...
    await submitTaskStart({
      ...
      request: async (form) => {
        ...
        const response = await humanMutationFetch("/api/tasks", "task-create", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ repoId: form.repoId, templateId: form.templateId, prompt: form.prompt }) });
...
function StartTaskPanel({ repos, repoId, templateId, prompt, sending, profileEnabled, selectedRepo, resolution, taskStartError, onRepo, onTemplate, onPrompt, onStart, onProjectAdded, onError, onOpenSettings }: { repos: Repo[]; repoId: string; templateId: string; prompt: string; sending: boolean; profileEnabled: boolean; selectedRepo?: Repo; resolution: "override" | "default" | "only_enabled" | "selection_required" | "none_enabled"; taskStartError: string; onRepo: (id: string) => void; onTemplate: (id: string) => void; onPrompt: (value: string) => void; onStart: () => void; onProjectAdded: (project: AddedProject, needsInitialCommit: boolean) => void; onError: (error: string) => void; onOpenSettings: () => void }) {
...
function TaskDetail({ task, tab, onTab, onDelete, onRun, busy }: { task: RepoTask; tab: "overview" | "changes" | "history" | "technical"; onTab: (tab: "overview" | "changes" | "history" | "technical") => void; onDelete: () => void; onRun: () => void; busy: boolean }) {
  ...
  return <section className="taskDetail card" aria-labelledby="task-detail-title"><div className="cardHeader"><div><span className="eyebrow">Task</span><h2 id="task-detail-title">{task.prompt}</h2></div><Status domain="task" value={task.status} /></div>
    {readOnly ? <p className="readOnlyNotice"><strong>{task.template.name} · Read-only</strong><br />No worktree, commit, or PR. Complete the review, then view findings.</p> : <p className="muted">Next action: <strong>{task.status === "draft" ? "Start review" : task.status === "awaiting_approval" ? "Review changes" : "Continue safely"}</strong></p>}
