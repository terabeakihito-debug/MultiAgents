...
export async function POST(request: Request) {
  const rejection = requireHumanMutation(request, "cleanup-execute", { label: "Cleanup" }); if (rejection) return rejection;
  ...
  try { return Response.json(await executeCleanup(Array.isArray(body.candidateIds) ? body.candidateIds as string[] : [])); }
