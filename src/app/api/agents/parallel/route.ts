...
import { agentIds } from "@/agents/types";
...
export async function POST(request: Request) {
  const rejection = requireHumanMutation(request, "agent-run", { label: "Parallel agent execution" });
  ...
  const results = await Promise.all(agentIds.map((id) => agents[id].run(safePrompt, { signal: request.signal, policy: buildGenericRuntimePolicy(id) })));
