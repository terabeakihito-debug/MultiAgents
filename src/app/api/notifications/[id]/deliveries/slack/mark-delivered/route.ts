...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const rejection = rejectNonHumanOutboundMutation(request, "outbound-mark-delivered"); if (rejection) return rejection;
  try { return Response.json({ delivery: markAmbiguousDelivery((await context.params).id, "delivered") }); }
