...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try { const finding = acceptFinding((await context.params).id); return Response.json({ finding, remediation: getRemediationFinding(finding.findingId), history: getStateStore().loadFindingEvents(finding.findingId) }); }
