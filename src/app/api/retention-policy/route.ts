...
export function GET(request: Request) {
  ...
  return Response.json(retentionPolicyService.load());
}

export async function POST(request: Request) {
  const rejection = requireHumanMutation(request, "retention-policy", { label: "Retention policy" });
  if (rejection) return rejection;
  let body: { preset?: unknown };
  try {
    body = await request.json();
  } catch {
    return Response.json({ error: "Retention policy must be valid JSON" }, { status: 400 });
  }
  if (!retentionPresets.includes(body.preset as RetentionPreset)) {
    return Response.json({ error: "Retention preset is invalid" }, { status: 400 });
  }
