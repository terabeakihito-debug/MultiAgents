...
export function codexArgs(prompt: string, cwd: string, repositoryTask: boolean, writeAccess = repositoryTask, model?: string) {
  ...
  return [
    ...
    "--ephemeral",
    ...(model ? ["-m", model] : []),
    ...(implement ? ["--dangerously-bypass-approvals-and-sandbox"] : ["--sandbox", "read-only"]),
    "--cd",
    cwd,
    prompt,
  ];
